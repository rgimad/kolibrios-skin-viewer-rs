...
pub fn skin_unpack(buffer: &[u8]) -> Result<Vec<u8>, Box<dyn Error> > {
    let mut cursor1 = Cursor::new(buffer);

    let _magic = cursor1.read_u32::<LittleEndian>()?;
    ...
    lzma_buf.extend_from_slice(&buffer[4*3..]);
